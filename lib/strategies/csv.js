...
export function prepareCsvItem(item, options) {
  ...
  if (!options.reverse && params.filters.citycode) {
    ...
    preparedItem[getKey('citycode')] = Array.isArray(params.filters.citycode)
      ? params.filters.citycode.join('+')
      : params.filters.citycode
  }

  // Not supported in reverse/csv
  if (!options.reverse && params.filters.postcode) {
    // Convert arrays to string with '+' as separator
    preparedItem[getKey('postcode')] = Array.isArray(params.filters.postcode)
      ? params.filters.postcode.join('+')
