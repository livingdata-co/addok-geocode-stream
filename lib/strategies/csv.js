...
export function prepareCsvItem(item, options) {
  ...
  for (const [filterName, filterValue] of Object.entries(params.filters)) {
    // Convert arrays to string with '+' as separator
    preparedItem[getKey(filterName)] = Array.isArray(filterValue)
      ? filterValue.join('+')
      : filterValue
  }
...
export async function geocodeMany(items, options) {
  ...
  // Append all filter columns dynamically
  if (options.filters) {
    for (const filterName of Object.keys(options.filters)) {
      formData.append(filterName, getKey(filterName))
    }
  }
